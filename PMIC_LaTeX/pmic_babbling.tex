...
\note[item]{When you start to feel sleepy tell me to throw something at you}
...
\note[item]{Formal me}
\note[item]{Linux drivers}
\note[item]{ROHM Semiconductor}
\note[item]{NOKIA clock and sync}
...
\note[item]{Try go fast so people have time to help me understand use cases}
	\begin{columns}
...
\note{This is what many people think powering a system is about.}
\note{Just plug-in the power to socket, whats the big deal?}
\begin{itemize}
	\item Processor and peripherals need power
	\item Can it be as simple as a dummy DC power source with correct voltage?
\end{itemize}
...
\note[item]{New design for each  board(?)}
	...
	\column{0.60\linewidth}\vspace{4pt}
...
\begin{columns}
\column{0.5\linewidth}
\note[item]<1>{Importance of power saving increases. My wife pays the electricity bill - she has told me this.}
...
%\note{More needs}
\note[item]<3>{battery powered devices everywhere, charging needs power}
\note[item]<3>{Watchdog almost every device has it. Power off}
\note[item]<3>{Monitor HW state. I have a feeling the importance of func safety increasing}
...
\note[item] {Recap: Multiple voltages}
\note[item] {Strict timing of sequences}
\note[item] {Power saving, wake-up, chargers, watchdog, HW state monitoring}
...
\note[item]{regulator in-kernel API to consumers}
...
\note[item]{proud of this slide. It pretty much requires no talking}
\note[item]{callbacks are registered to core usingg regulator ops struct}
\note[item]{shows just a part of functions}
...
\note[item]<1>{name}
\note[item]<1>{device-tree match information}
\note[item]<1>{supported voltages}
\note[item]<1>{register details for helper-callbacks}

\note[item]<2>{regulator-desc static config}
\note[item]<2>{regulator-config dynamic config like constrains, regmap, device}
\note[item]<2>{Then just register regulator using regulator register API and device is good to go}
\note[item]<2>{Thing to note is that typically PMICs have multiple regulators}
...
\note[item]{You can set limits for regulators.}
\note[item]{voltage area, boot-on, don't allow disabling, ...}
\note[item]{Regulator core enforce them. does not pass call to driver}
\note[item]{device tree, init data, core parses}
...
\begin{itemize}
	...
	\item other =\textgreater limit value
\end{itemize}
}
\end{frame}
...
\note[item]<1>{After core has parsed limits it needs to set them to HW}
\note[item]<1>{Driver provides the callbacks for that}
...
\note[item]<2>{The description with ops is passed to regulator registration}
...
\note{Work by Benjamin. Core uses these to disable monitoring when config takes place}
...
%		\item sent by provider (usually) from interrupt
		\item provider invokes consumer callback (blocking notifier call-chain)
		\item no polling needed
		\item in some cases IRQ is held active
		\item regulator$\_$register$\_$notifier()
		\item can send also other (non error) events
	\end{itemize}
}
%\end{itemize}
\end{columns}
...
\note[item]<1>{reread$\_$ms is time to wait after failed event mapping until retrying}
\note[item]<1>{irq$\_$off$\_$ms is time to keep IRQ disabled (before calling renable() if provided}
\note[item]<1>{skip$\_$off and high$\_$prio are something that were used in existing event implementations}
\note[item]<1>{skip$\_$off can be specified to not handle IRQs for regulators that are disabled}
\note[item]<1>{high$\_$prio can be set to use high$\_$priority work-queue for trying to re-enable the IRQ}
\note[item]<2>{data is pointer which can be used to store data needed in call-backs - like the driver private data}
\note[item]<2>{Callbacks for die(), mapping event and seeing if IRQ can be re-enabled. We will see the callbacks later}
...
\begin{frame}[fragile, t]{Helper Registration}
\note[item]{On top of this on the helper registration we provide}
\note[item]{device pointer and IRQ information}
\note[item]{errors that are common to all regulator devices this IRQ can be indicating errors for}
\note[item]{errors specific to only some of the regulator devices}
\note[item]{and finally an array of the regulator devices for which this IRQ can indicate problems}

\begin{itemize}
	\item IRQ information
	\item Array of regulators
	\item Events/Errors this IRQ can inform
...
\begin{frame}[fragile, t]{Re-enabling and simple mapping}


\note[item]<1>{driver can provide a renable() call-back, which should be checking if reported problem has gone and IRQ can be re-enabled}
\note[item]<1>{gets a pointer to the same struct regulator$\_$irq$\_$data as map$\_$event() did}
\note[item]<1>{simplest for just checks current status register value and compares to the opaque value}
\note[item]<1>{it is possible there is some new errors - or some old have gone while some have stayed. In this case the renable may prefer indicating that IRQ should be enabled. This will probably yield a new IRQ and updated set of notifications to be sent}

\note[item]<1>{finally, for simple HW where an IRQ can only signal one type of errors, on one regulator, driver can use generic helper called regulator$\_$irq$\_$map$\_$event$\_$simple()}
...
\begin{frame}[fragile]{Event mapping example part I}
...
\begin{frame}[fragile]{Event mapping example part II}
%\pause
\note[item]{Else, we will be setting the bits for failing regulators. The BD9576 status register directly has this information in correct format (bit 0 maps to first regulator, bit 1 in second and so on)}
\note[item]{finally, the notification and error flags are updated. The BD9576 uses same IRQ mechanism for WARNING and ERROR level notifications allowing the board designer to decide whether OVD should be treated as an ERROR or WARNING severity issue. The flags to advertise are then selected at probe time}
