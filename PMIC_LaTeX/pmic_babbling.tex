...
\note[item]{framework -> driver -> PMIC regs}
...
\note[item]<1>{Board specific -> via device-tree}
...
\note[item]<3>{Ask, What happens if silently ignored?}
\note[item]<3>{Ask, What happens if regulator registration fails?}
...
\note[item]<3>{In general, is there a way to mark NOT supported properties to binding docs? Would it help? Including ALL regulator bindings does hide this at a validation stage}
\note[item]{pause}
...
\note[item]<2>{name - obvious}
\note[item]<2>{we keep IRQs disabled for a second}
\note[item]<2>{We don't make this fatal or give a die() callback}
\note[item]<2>{populate map$\_$event and renable}
\lstset{language=C}
...
\note[item]<2>{in a loop where we register regulators, we add regulators which can inform OVD to the ovd$\_$devs array}
