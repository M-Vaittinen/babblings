...
%		\item regulator_get_error_flags()
	\end{itemize}
}
\onslide<3>{
...
\begin{columns}[onlytextwidth]
	...
	events
	\only<2->{
		\includegraphics[width=1\linewidth]{isr/IRQ.png}
	}
	\only<4->{
		\includegraphics[width=1\linewidth]{isr/emergpoff.png}
	}
	\only<5->{
		\includegraphics[width=1\linewidth]{isr/consumers.png}
	}
	\column[t]{0.25\linewidth}
	helper
	\includegraphics[width=1\linewidth]{isr/helperbox_size.png}
	\column[t]{0.20\linewidth}
	action \\[8pt]
	\only<3->{
		\includegraphics[width=1\linewidth]{isr/map_event.png}
	}
	\only<4->{
		\includegraphics[width=1\linewidth]{isr/die.png}\vspace{20pt}
	}
	\only<6->{
		\includegraphics[width=1\linewidth]{isr/renable.png}
	}
	\only<7->{
		\includegraphics[width=0.5\linewidth]{isr/retry.png}
	}
	\column[t]{0.30\linewidth}
	driver \\[4pt]
	\includegraphics[width=1\linewidth]{isr/driver_size.png}
%		\only<1> {\textbf{Why?} (I have 1 reason on mind, may be more)}
%		\only<2>{
%			\textbf{Allows re-use \\[10pt]}			
%		}
...
ret = devm_regulator_irq_helper(&pdev->dev, &bd9576_notif_ovd, irq,
		 0, ovd_errs, NULL,
